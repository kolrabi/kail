...
ILboolean iLoadPcdInternal(ILimage* image)
{
	...
	Total = Height >> 1;
	for (i = 0; i < Total; i++) {
		image->io.read(image->io.handle, Y1, 1, Width);
		image->io.read(image->io.handle, Y2, 1, Width);
		if (image->io.read(image->io.handle, CbCr, 1, Width) != Width) {  // Only really need to check the last one.
			ifree(Y1);
			ifree(Y2);
			ifree(CbCr);
			return IL_FALSE;
