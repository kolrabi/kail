...
static ILuint LifComputeCRC32(const ILubyte *data, ILuint length) {
	ILuint i, t;
	ILuint crc = 0xffffffff;

	for (i = 0; i<length; i++) {
		t   = (crc ^ data[i]) % 256;
		crc = ((crc >> 8) & 0x00FFFFFF) ^ LifCRCTable[t];
	}
	return(~crc);
}
...
static ILboolean iLoadLifInternal(ILimage *Image) {
	...
	SIOseek(io, LifHead.PalOffset, IL_SEEK_SET);
...
static ILboolean iSaveLifInternal(ILimage *Image) {
	ILuint		i;
	ILimage *TempImage;
	SIO *io;
	ILboolean isAlpha;
	...
	if (isAlpha) {
		...
		if (!iConvertImagePal(TempImage, IL_PAL_RGBA32)) {
			iCloseImage(TempImage);
			return IL_FALSE;
		}
	}

	SIOwrite(io, "Willy 7", 8, 1);
	SaveLittleUInt(io, 260);
	SaveLittleUInt(io, isAlpha ? LIF_Flag_Alpha : LIF_Flag_Palette);	// flags
	SaveLittleUInt(io, TempImage->Width);
	SaveLittleUInt(io, TempImage->Height);
	SaveLittleUInt(io, isAlpha ? 0 : LifComputeCRC32(TempImage->Pal.Palette, TempImage->Pal.PalSize)); // palette crc
	SaveLittleUInt(io, LifComputeCRC32(TempImage->Data, TempImage->SizeOfData )); // image crc
	SaveLittleUInt(io, isAlpha ? 0 : TempImage->SizeOfData + sizeof(LIF_HEAD)); // palette offset
	SaveLittleUInt(io, 0); // team effect offset 0
	SaveLittleUInt(io, 0); // team effect offset 1

	// Each data offset is offset by -1
	if (!isAlpha) {
	...
	if (TempImage->Origin == IL_ORIGIN_UPPER_LEFT) {
		SIOwrite(io, TempImage->Data, TempImage->SizeOfData, 1);
	} else {
		for (i=0; i < TempImage->Height; i++) {
			SIOwrite(io, TempImage->Data + (TempImage->Height - i - 1)*TempImage->Bps, TempImage->Bps, 1);
		}
	}
	if (!isAlpha)
		SIOwrite(io, TempImage->Pal.Palette, TempImage->Pal.PalSize, 1);
