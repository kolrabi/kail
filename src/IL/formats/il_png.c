...
ILboolean readpng_get_image(ILimage* image, struct PNGData * data, ILdouble display_exponent)
{
	...
	png_get_text(data->png_ptr, data->info_ptr, &text_ptr, &num_text);
	for (j = 0; j<num_text; j++) {
		if (text_ptr[j].key == NULL || text_ptr[j].text == NULL) continue;
...
//	png_init_io(png_ptr, PngFile);
	...
	if (image->Format == IL_COLOUR_INDEX) {
		...
		png_set_PLTE(png_ptr, info_ptr, (png_colorp)TempPal->Palette,
			(int)(TempPal->PalSize / iGetBppPal(IL_PAL_RGB24)));
