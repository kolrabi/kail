	...
	*/
	while (1) {
		...
		if( chunkInfo.tag == IFF_TAG_TBHD ) {
			// -- Header chunk found
			width = GetBigUInt(&iCurImage->io);
			height = GetBigUInt(&iCurImage->io);
			GetBigShort(&iCurImage->io);; // -- Don't support 
			GetBigShort(&iCurImage->io);; // -- Don't support 
			flags = GetBigUInt(&iCurImage->io);
			GetBigShort(&iCurImage->io);; // -- Don't support
			tiles = GetBigUShort(&iCurImage->io);
			compress	= GetBigUInt(&iCurImage->io);
	...
	while (!tileImageDataFound) {
		...
		while ( (tileImage < tiles) || (tileZ < tiles)) {
			...
			x1 = GetBigUShort(&iCurImage->io);	y1 = GetBigUShort(&iCurImage->io);
			x2 = GetBigUShort(&iCurImage->io);	y2 = GetBigUShort(&iCurImage->io);
...
iff_chunk iff_begin_read_chunk()
{
	...
	if (chunkStack[chunkDepth].tag == IFF_TAG_FOR4) {
		...
		chunkStack[chunkDepth].chunkType = GetBigInt(&iCurImage->io);
	} else {
...
char * iff_read_data(int size)
{
	...
	if (iCurImage->io.read(iCurImage->io.handle, buffer, size*sizeof(char), 1) != 1) {
		ifree(buffer);
		return NULL;
	}
...
char *iffReadUncompressedTile(ILushort width, ILushort height, ILbyte depth)
{
	...
	if (iCurImage->io.read(iCurImage->io.handle, data, tam, 1) != 1) {
		ifree(data);
		return NULL;
	}
