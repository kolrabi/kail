...
LZWInputStreamReadCode(
  ...
  while (Stream->InputBitCount < BitsLeftToRead) {
    ...
    while (Stream->InputAvail > 0 && Stream->InputBitCount <= 24) {
      Stream->InputBits |= (ILuint)(Stream->InputBuffer[Stream->InputPos] << Stream->InputBitCount);
      Stream->InputPos ++;
      Stream->InputAvail --;
...
GifLoadFrame(
  ...
  if (Ctx->UseLocalPal) {
    Ctx->Colors = (ILushort)( 1<<(1+(Img.Flags & GifFlag_IMG_LocalColorTableSizeMask)) );
  } else {
    Ctx->Colors = (ILushort)( 1<<(1+(Ctx->Screen.Flags & GifFlag_LSD_GlobalColorTableSizeMask)) );
  }
  ...
  while(LZWInputStreamReadByte(&Stream, &B) && interlaceGroup < 5) {
    ...
    if ( x < Ctx->Screen.Width 
      ...
      ILuint *UIntImageData = (ILuint*)(void*)Ctx->Image->Data;
      ILuint *UIntPaletteData = (ILuint*)(void*)SourcePal->Palette;
      UIntImageData[x + y * Ctx->Screen.Width] = UIntPaletteData[B];
...
static ILboolean iLoadGifInternal(ILimage* TargetImage)
{
  GifLoadingContext Ctx;
  GifSignature Sig;
    ILboolean Loaded;
...
iIsValidGif(SIO* io) {
  ...
  ILuint Read  = SIOread(io, &Sig, 1, 6);
  SIOseek(io, Start, IL_SEEK_SET);
  return Read == 6 && GifCheckHeader(&Sig);
}

static ILconst_string iFormatExtsGIF[] = { 
  IL_TEXT("gif"), 
  NULL 
};
