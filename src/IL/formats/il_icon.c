...
ILboolean iLoadIconInternal(ILimage* image)
{
	...
	IconDir.Reserved = GetLittleShort(&iCurImage->io);
	...
	for (i = 0; i < IconDir.Count; ++i) {
		DirEntries[i].Width = (ILubyte)iCurImage->io.getchar(iCurImage->io.handle);
		DirEntries[i].Height = (ILubyte)iCurImage->io.getchar(iCurImage->io.handle);
		DirEntries[i].NumColours = (ILubyte)iCurImage->io.getchar(iCurImage->io.handle);
		DirEntries[i].Reserved = (ILubyte)iCurImage->io.getchar(iCurImage->io.handle);
	...
	for (i = 0; i < IconDir.Count; i++) {
		...
		iCurImage->io.getchar(iCurImage->io.handle); // Skip the first character...seems to vary.
		...
		else
		{
			...
			IconImages[i].Head.Height = GetLittleInt(&iCurImage->io);
			IconImages[i].Head.Planes = GetLittleShort(&iCurImage->io);
			IconImages[i].Head.BitCount = GetLittleShort(&iCurImage->io);
			IconImages[i].Head.Compression = GetLittleInt(&iCurImage->io);
			IconImages[i].Head.SizeImage = GetLittleInt(&iCurImage->io);
			IconImages[i].Head.XPixPerMeter = GetLittleInt(&iCurImage->io);
			IconImages[i].Head.YPixPerMeter = GetLittleInt(&iCurImage->io);
			...
			if (IconImages[i].Head.BitCount < 8) {
				...
				if (iCurImage->io.read(iCurImage->io.handle, IconImages[i].Pal, IconImages[i].Head.ColourUsed * 4, 1) != 1)
					goto file_read_error;
			}
				...
				if (IconImages[i].Pal == NULL)
					goto file_read_error;
				if (iCurImage->io.read(iCurImage->io.handle, IconImages[i].Pal, 1, 256 * 4) != 256*4)
					goto file_read_error;
			}
			else {
			...
			if (IconImages[i].Data == NULL)
				goto file_read_error;
			if (iCurImage->io.read(iCurImage->io.handle, IconImages[i].Data, 1, Size) != Size)
				goto file_read_error;
			...
			if (IconImages[i].AND == NULL)
				goto file_read_error;
			if (iCurImage->io.read(iCurImage->io.handle, IconImages[i].AND, 1, Size) != Size)
				goto file_read_error;
		}
	}
