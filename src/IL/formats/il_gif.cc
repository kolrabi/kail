...
ILint get_next_code(GifLoadState* state) {
	...
	//crashed sometimes if iCurImage->io.getchar(iCurImage->io.handle) returned IL_EOF
	...
	if (state->nbits_left == 0) {
		if (state->navail_bytes <= 0) {
			state->resetBufIndex();
			state->navail_bytes = iCurImage->io.getchar(iCurImage->io.handle);
			...
			if (state->navail_bytes) {
				for (i = 0; i < state->navail_bytes; i++) {
					if((t = iCurImage->io.getchar(iCurImage->io.handle)) == IL_EOF) {
						return state->ending;
					}
   ...
   GFXCONTROL *Gfx)
{
	...
	if((size = iCurImage->io.getchar(iCurImage->io.handle)) == IL_EOF)
		return IL_FALSE;
...
ILboolean GetImages(ILimage* Image, ILpal *GlobalPal, GIFHEAD *GifHead)
{
	...
	while (!iCurImage->io.eof(iCurImage->io.handle)) {
      ...
      if (!setMinSizes(ImageDesc.OffX + ImageDesc.Width, ImageDesc.OffY + ImageDesc.Height, Image))
		...
		if((input = iCurImage->io.getchar(iCurImage->io.handle)) == IL_EOF)
			goto error_clean;
...
ILboolean SkipExtensions(GFXCONTROL *Gfx)
{
	...
	do {
		if((Code = iCurImage->io.getchar(iCurImage->io.handle)) == IL_EOF)
			return IL_FALSE;
		...
		if((Label = iCurImage->io.getchar(iCurImage->io.handle)) == IL_EOF)
			return IL_FALSE;

		switch (Label)
		{
			case 0xF9:
				Gfx->Size = iCurImage->io.getchar(iCurImage->io.handle);
				Gfx->Packed = iCurImage->io.getchar(iCurImage->io.handle);
				Gfx->Delay = GetLittleUShort(&iCurImage->io);
				Gfx->Transparent = iCurImage->io.getchar(iCurImage->io.handle);
				Gfx->Terminator = iCurImage->io.getchar(iCurImage->io.handle);
				if (iCurImage->io.eof(iCurImage->io.handle))
					return IL_FALSE;
				Gfx->Used = IL_FALSE;
				break;
