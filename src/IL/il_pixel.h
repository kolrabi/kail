...
#define iPixelConv1f(Tfrom, From, MaxFrom, Tto, To, MaxTo) \
    ((Tto*)To)[0] = (Tto)IL_CLAMP_FLOAT( ((Tfrom*)From)[0] / (float)MaxFrom) * (MaxTo);

#define iPixelConv2f(Tfrom, From, MaxFrom, Tto, To, MaxTo) \
    ((Tto*)To)[0] = (Tto)IL_CLAMP_FLOAT( ((Tfrom*)From)[0] / (float)MaxFrom) * (MaxTo); \
    ((Tto*)To)[1] = (Tto)IL_CLAMP_FLOAT( ((Tfrom*)From)[1] / (float)MaxFrom) * (MaxTo);

#define iPixelConv3f(Tfrom, From, MaxFrom, Tto, To, MaxTo) \
    ((Tto*)To)[0] = (Tto)IL_CLAMP_FLOAT( ((Tfrom*)From)[0] / (float)MaxFrom) * (MaxTo); \
    ((Tto*)To)[1] = (Tto)IL_CLAMP_FLOAT( ((Tfrom*)From)[1] / (float)MaxFrom) * (MaxTo); \
    ((Tto*)To)[2] = (Tto)IL_CLAMP_FLOAT( ((Tfrom*)From)[2] / (float)MaxFrom) * (MaxTo);

#define iPixelConv3Swapf(Tfrom, From, MaxFrom, Tto, To, MaxTo) \
    ((Tto*)To)[2] = (Tto)IL_CLAMP_FLOAT( ((Tfrom*)From)[0] / (float)MaxFrom) * (MaxTo); \
    ((Tto*)To)[1] = (Tto)IL_CLAMP_FLOAT( ((Tfrom*)From)[1] / (float)MaxFrom) * (MaxTo); \
    ((Tto*)To)[0] = (Tto)IL_CLAMP_FLOAT( ((Tfrom*)From)[2] / (float)MaxFrom) * (MaxTo);

#define iPixelConv4f(Tfrom, From, MaxFrom, Tto, To, MaxTo) \
    ((Tto*)To)[0] = (Tto)IL_CLAMP_FLOAT( ((Tfrom*)From)[0] / (float)MaxFrom) * (MaxTo); \
    ((Tto*)To)[1] = (Tto)IL_CLAMP_FLOAT( ((Tfrom*)From)[1] / (float)MaxFrom) * (MaxTo); \
    ((Tto*)To)[2] = (Tto)IL_CLAMP_FLOAT( ((Tfrom*)From)[2] / (float)MaxFrom) * (MaxTo); \
    ((Tto*)To)[3] = (Tto)IL_CLAMP_FLOAT( ((Tfrom*)From)[3] / (float)MaxFrom) * (MaxTo); 
...
#define iPixelConv3Lf(Tfrom, From, MaxFrom, Tto, To, MaxTo) \
    ((Tto*)To)[0] = (Tto)IL_CLAMP_FLOAT( ( ((Tfrom*)From)[0] / (float)MaxFrom) * 0.212671f + \
...
#define iPixelConv3LSwapf(Tfrom, From, MaxFrom, Tto, To, MaxTo) \
    ((Tto*)To)[0] = (Tto)IL_CLAMP_FLOAT( ( ((Tfrom*)From)[2] / (float)MaxFrom) * 0.212671f + \
                                    ( ((Tfrom*)From)[1] / (float)MaxFrom) * 0.715160f + \
...
#define iPixelConv4LAf(Tfrom, From, MaxFrom, Tto, To, MaxTo) \
    ((Tto*)To)[0] = (Tto)IL_CLAMP_FLOAT( ( ((Tfrom*)From)[0] / (float)MaxFrom) * 0.212671f + \
                                    ( ((Tfrom*)From)[1] / (float)MaxFrom) * 0.715160f + \
                                    ( ((Tfrom*)From)[2] / (float)MaxFrom) * 0.072169f   ) * (MaxTo); \
    ((Tto*)To)[1] = (Tto)IL_CLAMP_FLOAT( ((Tfrom*)From)[3] / (float)MaxFrom) * (MaxTo); 
