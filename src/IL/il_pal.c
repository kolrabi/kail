...
// FIXME: change to not set iCurImage, add iLoadPal, iLoadImage, etc that accept an ILimage *
ILboolean ILAPIENTRY ilApplyPal(ILconst_string FileName)
{
	ILimage		Image, *CurImage = iGetCurImage();
	ILubyte		*NewData;
  ...
  if( CurImage == NULL || (CurImage->Format != IL_BYTE || CurImage->Format != IL_UNSIGNED_BYTE) ) {
	...
	// HACK to load a palette directly into an image...
	ilSetCurImage(&Image); 
